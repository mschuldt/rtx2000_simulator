...
$A000 $4000 $FF FILL   $A200 H' ! 0 RAM !
...
: UM/MOD ( ud u - ur uq)
  MD! D2*
  U/1' U/' U/' U/' U/' U/' U/' U/' U/' U/' U/' U/' U/' U/' U/' U/"  SWAP ;
...
\ ----------
 \
  \ the m/mod operator returns signed quotients and remainders as:
  \
  \       -a -b  ==  -r +q
  \       -a +b  ==  -r -q
  \       +a -b  ==  +r -q
  ...
  \ faster.  It is, however, intuitionally correct.
  \ [ a / b = q r ] and [ q * b + r = a ]  for all cases.

: M/MOD ( a. b -- r q)   \ "a" is signed double, b is signed single
   >R
   DUP_0< IF ( -a)
      DNEGATE  R>
      DUP_0< IF ( -a -b)
         NEGATE  UM/MOD/SWAP ( uq ur)
         NEGATE SWAP EXIT
      THEN ( -a +b)
      UM/MOD/SWAP ( uq ur)
      NEGATE SWAP NEGATE EXIT
   THEN ( +a)
   R>
   DUP_0< IF ( +a -b)
      NEGATE UM/MOD/SWAP ( uq ur)
      SWAP NEGATE EXIT
   THEN ( +a +b)
   UM/MOD/SWAP ( uq ur) SWAP ;

\ ----------

: /MOD  ( a b -- r q )    >R  DUP_0<  R>  M/MOD ;
: */MOD ( a b c -- r q )  >R  M*      R>  M/MOD ;
...
: MIN ( a b -- n )  DDUP > IF NIP EXIT  THEN  DROP ;
: MAX ( a b -- n )  DDUP < IF NIP EXIT  THEN  DROP ;
...
{ : FILE-TO-LOAD ( -- handle) }
   USING $USING COUNT OPEN-FILE  DUP huh? ;

: LLOAD ( line#)
   FILE-TO-LOAD
   $2C CMD  DUP ( handle) XMT   SWAP  DUP ( line#) 1- #XMT
   RCV ( handshake) DROP  ( handle line#) FLOAD ;

: LOAD
   FILE-TO-LOAD 0 FLOAD ;
