...
\ set to 0 for simulator running on a Sparcstation, 1 for PC
1 CONSTANT _TURBOC              IMMEDIATE

\ compilation cpu type
1 CONSTANT _RTX2000             IMMEDIATE
0 CONSTANT _RTX2001A            IMMEDIATE
0 CONSTANT _RTX2010             IMMEDIATE
...
_RTXDB _TURBOC [ and ] #IF
            " $A000 $4000 SAVE-IMAGE APPFORTH.BIN" .EVALUATE  #END
_RTXDB _TURBOC [ 0= and ] #IF
            " $A000 $4000 SAVE-IMAGE SUNFORTH.BIN" .EVALUATE  #END
   THEN ;
...
\ ---------- rtx 2000 series divide routine,
 \
  \ COPYRIGHT 1990 HARRIS CORPORATION  ALL RIGHTS RESERVED
  \ Tue  09-25-1990  15:34:00
  \
  \ by Rick VanNorman
  \
...
\ using this divide primitive saves 54 words of program space in the
\ x/mod routine and costs 2 clock cycles of overhead.  to reclaim
\ the 2 clock cycles, simply code this routine inline in x/mod
: UM/MOD/SWAP ( ud u -- uq ur )
   MD! D2*
   U/1' U/' U/' U/' U/' U/' U/' U/' U/' U/' U/' U/' U/' U/' U/' U/"
   NOP ; ( 20)

\ ----------
 \
  \ the x/mod operator returns signed quotients and remainders as:
  \
  \       -a -b  ==  -r +q      32 clocks
  \       -a +b  ==  -r -q      32 clocks
  \       +a -b  ==  +r -q      31 clocks
  \       +a +b  ==  +r +q      29 clocks
  \
  \ this is _not_ floored division, but is hardware specific and much
  \ faster.
...
: /MOD ( a b -- r q )
   DUP_0< IF ( -b)
      NEGATE >R
      DUP_0< IF ( -a)
         NEGATE 0 R>  UM/MOD/SWAP ( uq ur)
         NEGATE SWAP ( -r +q) EXIT ( 32 clocks)
      THEN ( +a)
      0 R> UM/MOD/SWAP ( uq ur)
      SWAP NEGATE ( +r -q) EXIT ( 31 clocks)
   THEN ( +b)
   >R
   DUP_0< IF ( -a)
      NEGATE 0 R>  UM/MOD/SWAP ( uq ur)
      NEGATE SWAP NEGATE ( -r -q) EXIT ( 32 clocks)
   THEN ( +a)
   0 R> UM/MOD/SWAP SWAP ( +r +q) ; ( 29 clocks)
...
: RCV ( - c)    BEGIN RCV?  UNTIL  $19 G@ ;
: XMT ( c -)    BEGIN XMT?  UNTIL  $19 G! ;
...
: CR   _TURBOC #IF 13 EMIT #END 10 EMIT  0 #OUT ! ;
...
{ : STROKE } ( ... col char - col)
   DDUP  _TURBOC #if DUP EMIT #END DROP   HERE + C!  1+ ;
...
: WORDS
   ...
   CR BEGIN  
            _TURBOC  #IF  ?PAUSE  #END
            ANOTHER ?DUP WHILE ?.NAME REPEAT
...
: DUMPL ( addr page #)
   BASE @ >R  HEX
      ...
      16 /  0 MAX  FOR ( addr page )
         _TURBOC #IF ?PAUSE #END  DDUP DUMP-LINE  16. D+
      NEXT  DDROP
   R> BASE ! ;
...
{ : .HDR }
   CR
     LOCATION @   4 N.          124 _TURBOC #IF 55 + #END EMIT
     THIS         4 N.          124 _TURBOC #IF 55 + #END EMIT
     LOCATION @ 0 1 DUMP-ASCII  124 _TURBOC #IF 55 + #END EMIT ;
...
$BE07 UCODE PC@
$BE00 UCODE R@
$BE87 UCODE RH!     \  Bug in AppForth??? this is an execute instruction
$BE17 UCODE RH@
$BE86 UCODE RX!
$BE16 UCODE RX@
$A0C1 UCODE S>D
...
_TURBOC #IF dos unload appforth.bin
        #END
_TURBOC 0= #IF dos unload sunforth.bin
           #END
bye
